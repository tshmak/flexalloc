getutil2 <- function(params, ..., npoints=200) {

  # Options needed: V1, Maxx1, Beta1, b, maxx, sumx, k, bmax, slope
  o <- list(...)
  ...
  util1 <- with(o, {
    ...
    if(k < Inf) {
      Pay <- MyersonPay(b, X2, sumx=sumx, maxx=maxx, k=k, f=ffun, F=Ffun,
                         bmax=bmax, subset=1, npoints=npoints, integration="rectangle")
    } else {
    ...
    xhat1 <- x1 * xhat1prop
    cost1 <- xhat1*V1
    dw1 <- downweight2(xhat1, Maxx1=Maxx1, slope=slope) 
